...
def get_spreadsheet():
    ...
    # ตรวจสอบว่ามีข้อมูลใน Streamlit Secrets หรือไม่
    if "gcp_service_account" in st.secrets:
        # ดึงข้อมูลจากหน้า Settings > Secrets ของ Streamlit Cloud
        creds_info = dict(st.secrets["gcp_service_account"])
        # สำคัญ: ต้องจัดการเรื่องการขึ้นบรรทัดใหม่ใน Private Key
        creds_info["private_key"] = creds_info["private_key"].replace("\\n", "\n")
        creds = Credentials.from_service_account_info(creds_info, scopes=scopes)
    else:
        # กรณีรันในเครื่อง (Local) ให้ใช้ไฟล์เดิม
